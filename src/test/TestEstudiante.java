...
public class TestEstudiante {
    ...
    public static void mayorMenor(Estudiante est[],int n){
            char sexo;
            String nombre;
            double notas;
            for(int i=0;i<n;i++){
                for(int k=0;k<n-1;k++){
                    if(est[k].getNota()<est[k+1].getNota()){
                        nombre=est[k+1].getNombre();
                        sexo=est[k+1].getSexo();
                        notas=est[k+1].getNota();
                        est[k]=est[k+1];
                        est[k]=new Estudiante(nombre, sexo, notas);
                    }
                }
            }
    }
